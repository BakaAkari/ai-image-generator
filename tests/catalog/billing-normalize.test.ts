...
import { normalizeYunwuBilling, PLATFORM_CREDIT_TO_RMB, SUPPLIER_CREDIT_TO_RMB } from '../../src/catalog/billing-info.js'

describe('normalizeYunwuBilling naming and compat', () => {
  const baseSnapshot = {
  ...
  it('exposes supplierCredits = total_usage / 500000 and keeps the legacy alias', () => {
    const info = normalizeYunwuBilling(baseSnapshot)
  ...
  it('exports the fixed 0.5 yunwu credit→RMB rate as a constant', () => {
  ...
  it('handles missing usage gracefully (null credits, null legacy alias)', () => {
    ...
    const info = normalizeYunwuBilling(snap)
