...
describe('normalizeV1Base', () => {
  test('appends /v1 to a bare host', () => {
    expect(normalizeV1Base('https://yunwu.ai')).toBe('https://yunwu.ai/v1')
  })
  ...
  test('is idempotent when /v1 is already present', () => {
    expect(normalizeV1Base('https://yunwu.ai/v1')).toBe('https://yunwu.ai/v1')
  })
  ...
  test('composed generations URL has no doubled /v1', () => {
    const apiBase = normalizeV1Base('https://yunwu.ai')
    expect(`${apiBase}/images/generations`).toBe('https://yunwu.ai/v1/images/generations')
  })
})
