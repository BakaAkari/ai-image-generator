...
自用 AI 图像生成插件 V2（image-only）。当前 `0.7.0` 版本采用供应商凭证 + 模型路由配置界面，支持在每条模型映射中显式选择供应商、运行时协议与积分单价，并使用积分制计费、用户数据 v2、精简后的账户命令体系，以及 ChatLuna 桥接集成（可选）。
...
- `0.7.x`：等待远端验证 ChatLuna 桥接集成的工具注册、工具调用、上下文注入和配置热重载表现，根据日志修复问题。
