...
- **图生图 style 预设入口不先收图**：带锁定 prompt 的 image-to-image 预设（如「变手办」）此前跳过收图直接进模型选择，用户选完模型/参数到生成才发现没图。现与「图生图」命令一致：进入即提示「请先发送 1 张图片」，再收描述、再选模型。

### 调整

- **向导模型列表只显示 modelId + 成本**：移除 `[OPENAI]` 协议段、`-gpt` 命令后缀、`异步 30-120s` 标签。示例：`1 · gpt-image-1 ~0.62 积分`。用户无需感知响应协议与命令后缀。

### 验证

- `tsc` clean；全量 `619 passed`（wizard-dependent-resolution 断言更新为 modelId 格式）。

## [2.6.8] - 2026-09-01

### 修复

- **严重：引导向导第一步之后的所有回复在 QQ 侧完全丢失**（影响 configured=guided 与 auto 解析为 guided 的全部流程）。根因：Koishi v4 中间件返回字符串只表示拦截消息、**不会自动发送**；向导中间件此前的字符串回复全部静默落空，只有命令回调返回的首条回复（模型列表/请发送画面描述）能送达。用户表现为「发了描述后 bot 无反应」，实际是分辨率/参数/确认等每一步的回复都未投递。修复：中间件统一出口改为显式 `session.send()` 发送后返回 void 拦截；发送失败记 warn 不再静默。经 NapCat 侧日志交叉验证：修复前中间步骤消息从未到达 NapCat。

### 验证

- `tsc` clean；全量 `619 passed`（wizard 全部测试断言改为校验 session.send 通道）。
- 复现链路：QQ 私聊 MC → 1/1/1 推进，NapCat 日志确认仅首条回复投递。

## [2.6.7] - 2026-09-01

### 新增

- **向导每步回复增加步骤上下文前缀**：非终态回复统一冠以「〔第N步 · 步骤名〕 · 已记录描述「…」」前缀，覆盖模型列表/分辨率/参数/确认/帮助/上一步/图片更新等全部中间件回复。解决「提示文本与向导状态错位」——用户在 model-select 步重发描述时，回复会明确显示当前在选择模型且描述已记录，不再产生「bot 没反应」的错觉。确认/取消等终态回执不加前缀。

### 验证

- `tsc` clean；全量 `619 passed`（wizard 相关断言更新为包含式匹配）。

## [2.6.6] - 2026-08-31

### 新增

- **detail 日志级别覆盖引导与编排全链路**：新增结构化诊断日志（仅 `logLevel: detail` 输出，simple 完全静默）：
  - 向导层（wizard-handler）：会话启动/冲突、超时回收、收到输入，以及每次用户操作的结果（取消/上一步/帮助/更新图片/输入描述/选择模型/选择分辨率/设置参数/确认步骤），均带 scope/step/mode 上下文。
  - 编排层（orchestrator）：限流拒绝、任务锁冲突、任务开始/结束（含耗时）、模型目录未就绪、积分预授权/拒绝（含估价与计价路径）、高级模式等待用户输入的结果（message/interrupted/cancelled/timeout）。
  - 命令入口（commands/image）：交互模式解析结果（配置值/平台覆盖/实际生效/群聊判定/直接语法命中），覆盖文生图/图生图/合成图/style 四类命令。
- 移除此前临时接入的 `[wizard-dbg]` 调试日志。

### 验证

- `tsc` clean；全量 `619 passed`。
- `pnpm build` 通过；本地 koishi-app 重载新构建，插件加载无报错，simple 级别下无新增日志输出。
...
- **简易模式恢复「预设」入口标签**：`SIMPLE_TABS` 标签集合此前遗漏 `presets`，切换/降级到简易模式（simple）后顶部标签栏不再渲染「预设」按钮，导致 Prompt 预设管理区块（后台分类管理）从界面消失。已在简易模式标签集合中补回 `{ key: 'presets', label: '预设' }`（置于「模型定价」之后、「运营」之前，与自动模式标签集合对齐）。预设面板本身在 simple 模式下一直存在、未被功能屏蔽，仅缺入口按钮；`onConfigModeChange` 切换时已重置 `activeTab` 到总览，补回标签安全、无空白页风险。

### 验证

- `tsc` clean；`pnpm build` 通过（tsup + koishi-console build），已确认修复写入 `dist/index.js` bundle（简易模式标签序列：`模型定价 → 预设 → 运营`）。
- 注：2.6.1 的「验证」曾记录切 simple 后 tabs 5→4 属预期收敛；本次按新需求恢复 simple 下预设入口，非回归回退。
