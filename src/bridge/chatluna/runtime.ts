/**
 * ChatLuna 运行时动态加载器（V2 移植版）。
 *
 * 通过 createRequire + 动态 import 双通道加载 koishi-plugin-chatluna
 * 和 @langchain/core/tools，避免将 ChatLuna 设为硬依赖。
 */
...
export type StructuredToolConstructor = new (...args: any[]) => {
  ...
  _call(
    ...
    runManager?: unknown,
    config?: { configurable?: { session?: unknown } },
  ): Promise<string>
}
