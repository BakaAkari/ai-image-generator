/**
 * ChatLuna 上下文注入（V2 积分制适配版）。
 *
 * 在 chatluna/before-chat 事件中注入 AIGC 上下文：
 * - 上一张图像的引用信息（[AIGC_CONTEXT]）
 * - 风格推荐候选（[AIGC_STYLE_CANDIDATES]）
 * - 同时注入到 promptVariables 中供模板使用
 *
 * 在 chatluna/clear-chat-history 事件中清除对应的图像上下文。
 *
 * 关键 V2 适配：
 * - AiImageGeneratorService 的 pruneConversationImageContexts / getConversationImageContext
 *   / clearConversationImageContext 方法名与 V1 一致，无需修改调用。
 * - matchStylePresets 返回 StyleMatchCandidate[]，与 V1 的接口兼容。
 */
...
export function installChatLunaContextInjection(
  ...
  const disposeBeforeChat = (ctx as Context & {
    ...
    const shouldInjectImageContext =
      looksLikeImageFollowUp(text) && Boolean(conversationId)

    if (!shouldInjectStyleCandidates && !shouldInjectImageContext) return

    const context = conversationId
      ? aiGenerator.getConversationImageContext(conversationId)
      : undefined
    ...
    if (promptVariables) {
      ...
      promptVariables.aiGeneratorPreferredStylePreset =
        styleMatches[0]?.style.commandName || ''
    ...
    logger.debug(
      'aigc context injected for conversationId=%s with %s style candidates',
      conversationId || 'n/a',
      styleMatches.length,
    )
  })
...
function extractConversationId(
  ...
  const direct =
    readConversationId(conversationArg) ||
    readConversationId(promptVariables) ||
    readConversationId(session)
  const platform =
    readPlatform(conversationArg) ||
...
function asSession(input: unknown) {
  return input && typeof input === 'object' ? (input as ChatLunaSessionLike) : null
}
...
function setAugmentedMessageContent(
  ...
  if (Array.isArray(originalContent)) {
    message.content = [{ type: 'text', text: prefix }, ...originalContent]
    return
  }
...
function hasAnyAiGeneratorContextBlock(text: string) {
  return (
    text.includes('[AIGC_CONTEXT]') ||
    text.includes('[AIGC_STYLE_CANDIDATES]')
  )
}

function formatStyleCandidatesBlock(
  matches: ReturnType<AiImageGeneratorService['matchStylePresets']>,
) {
  ...
  for (const [index, item] of matches.entries()) {
    ...
    if (item.style.aliases?.length)
      lines.push(`aliases: ${item.style.aliases.join(', ')}`)
    if (item.style.keywords?.length)
      lines.push(`keywords: ${item.style.keywords.join(', ')}`)
    if (item.style.whenToUse) lines.push(`whenToUse: ${item.style.whenToUse}`)
    if (item.matchedTerms.length)
      lines.push(`matchedTerms: ${item.matchedTerms.join(', ')}`)
  }
  lines.push(
    'instruction: If the user seems to reference one of these configured styles, prefer aigc_apply_style_preset and pass stylePreset exactly, or pass styleQuery when uncertain.',
  )
