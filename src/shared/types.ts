 ...
 * V2 共享类型定义。
 *
 * 注意：v1 中此文件 `import type { ProviderType } from '../providers'`，
 * 但 v2 的 providers 层会在 Phase 2 完整重写。为打破循环，本文件先以
 * `string` 占位 ProviderType，Phase 2 再将其替换为正式 ProviderRegistry 的类型。
 */

// V2 Phase 1 占位：Phase 2 重写为 ProviderRegistry 的强类型 union
export type ProviderType = string

export type ImageProvider = 'yunwu' | 'gptgod' | 'gemini' | 'grok' | 'openai'
